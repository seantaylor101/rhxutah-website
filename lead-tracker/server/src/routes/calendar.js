...
router.get("/leads/:id.ics", requireAuth("viewer"), (req, res) => {
  ...
  const ics = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//RHX Job Board//Appointments//EN", buildEvent(lead), "END:VCALENDAR"].join(
    "\r\n"
  );
...
router.get("/feed/:token.ics", (req, res) => {
  ...
  const leads = db
    ...
    .all();
  const ics = buildCalendar(leads, { name: "RHX Appointments" });
  res.set("Content-Type", "text/calendar; charset=utf-8");
  res.send(ics);
});
